function [X] = LocalizeLowRankSDP_3D_RPCA(rx,ra,d,nt,A,cons,X0,W0,Display,lambda,sigma)
%
...
%       sure the U0, S0 and V0 have enough number of rows and columns
  ...
  if (lrank > 0)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%extend variables%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %   \xi \|A + E - X0\|_F^2 + \sigma \|E\|_F^2 + \lambda \|A - X0\|_F^2
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  xi = 10;
  ncmx = rx * d * nt;
  nall = ncmx * 2 + (rx*d+nt) * lrank;
  nsqu = rx*nt + rx*d + nt;
  
  
  x_size = [x_size ; repmat([rx d],nt,1)];
  x0 = [x0 ; zeros(ncmx,1)];
  x_reg = [x_reg ; zeros(ncmx,1)];
  w0 = [w0 ; zeros(ncmx,1)];
  G = [G sparse(zeros(size(G,1),ncmx))];
  temp = sparse(zeros(nsqu + rx*nt));
  for c = 1:length(H)
      temp(1:nsqu, 1:nsqu) = H{c}; 
      H{c} = temp;
  end  
  %
  % objective:
...
%     iter
    ...
    temp = zeros(nall);
    temp1 = temp;
    temp1(1:ncmx,1:ncmx) = eye(ncmx);
    temp1(end-ncmx+1:end,end-ncmx+1:end) = eye(ncmx);
    temp1(1:ncmx,end-ncmx+1:end) = eye(ncmx);
    temp1(end-ncmx+1:end,1:ncmx) = eye(ncmx);
    temp2 = temp;
    temp2(end-ncmx+1:end,end-ncmx+1:end) = eye(ncmx);
    
    temp3 = x_reg;
    temp3(ncmx+1:end-ncmx) = zeros((rx*d+nt) * lrank,1);
    temp3(end-ncmx+1:end) = x_reg(1:ncmx);
    % form P and q
%     P = lambda*speye(nX,nX) + R'*R;
%     q = -2*lambda*x_reg;
    P = sqrt(xi)*temp1 + sqrt(sigma)*temp2 + lambda*sparse(diag(w0)) + R'*R ; % revised by jie
    q = -2*xi*temp3 - 2*lambda*(x_reg.*w0);

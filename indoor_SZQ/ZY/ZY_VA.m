function [err1,err2,varargout]=ZY_VA(Network,X,Y,ErrorName)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
disp([' ']);
disp(['     Enter ZY_VA --> Hole:',num2str(Network.HoleNum),' || ra Step ',num2str(Network.raStep),' || Repeat Times ',num2str(Network.time),'.']);
if nargin==0
...
for k=1:Network.raStep
    err{k}=zeros(Network.time,Network.T);
    Network.ra=Network.ras(k);
    Network.rx = Network.N - Network.ra;
    Network.currentRatio = Network.ra;
    for i=1:Network.time
        Network.round = i;
        ...
        showtime(['         Repeat Time: ',num2str(i),...
            ' || ra: ',num2str(Network.ras(k)),' || Algorithm: ',alname,' || Finished at:']);
    end
    err1(k,:)=mean(err{k}(:,end-9:end),2);
    showtime(['------------> ra: ',num2str(Network.ras(k)),' || Algorithm: ',alname,' ||  Finished at:']);
end

...
for i=1:L
    dire_vec(i,:)=G_rotated(1+fix(Y(i)/n),:)-G_rotated(mod(Y(i),n),:);
    dire_vec0(i,:)=G_ini(1+fix(Y(i)/n),:)-G_ini(mod(Y(i),n),:);
    dire_vec1(i,:)=G_rotated0(1+fix(Y(i)/n),:)-G_rotated0(mod(Y(i),n),:);
end
